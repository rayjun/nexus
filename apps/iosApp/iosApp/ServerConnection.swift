...
final class ServerConnection: NSObject, URLSessionWebSocketDelegate {
    ...
    func connect() {
        ...
        let d = ConnDelegate()
        d.onOpen = { [weak self] in self?.onConnected() }
        d.onClose = { [weak self] in self?.onDisconnected() }
    ...
    private func handleAgentPublicKey(_ payload: String) {
        guard let agentPub = Data(base64Encoded: payload),
              let shared = E2ECrypto.computeSharedSecret(myPriv: keyPairPriv, peerPub: agentPub),
